...
function start(): void {
	...
	state.currentDuration = props.duration * 1000;
	state.paused = false;
	this.animationFrame = window.requestAnimationFrame(counting);
}
function reset(): void {
	state.paused = false;
	state.startTimestamp = null;
...
function counting(timestamp: number): void {
	...
	if (!this.isCountingUp) {
		state.currentAmount =
			state.currentStartAmount - (state.currentStartAmount - this.endAmount) * (progress / state.currentDuration);
		state.currentAmount = state.currentAmount < this.endAmount ? this.endAmount : state.currentAmount;
	} else {
		state.currentAmount =
			state.currentStartAmount + (this.endAmount - state.currentStartAmount) * (progress / state.currentDuration);
		state.currentAmount = state.currentAmount > this.endAmount ? this.endAmount : state.currentAmount;
	}

	if (progress < state.currentDuration) this.animationFrame = window.requestAnimationFrame(counting);
